...
public class Add extends JFrame { //Add 클래스는 jframe에 상속
	...
	List<Student> sList; 	   //List<Student>선언
	private JTextField tfEng; //영어
	Student s;


	
	public Add(List<Student> sList) { //Add생성자 선언
		...
		btnInsert.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				s = new Student(); //객체선언
				s.setName(tfName.getText()); //Student의 setName을 불러옴
				s.setKor(Integer.parseInt(tfKor.getText())); //Student의 setName을 불러옴
				s.setMath(Integer.parseInt(tfMath.getText())); //Student의 setName을 불러옴
				s.setEng(Integer.parseInt(tfEng.getText())); //Student의 setName을 불러옴
